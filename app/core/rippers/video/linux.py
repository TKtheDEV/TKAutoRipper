...
def rip_video_disc(job: Job, disc_type: str) -> List[Tuple[List[str], str, bool, float]]:
    ...
    if use_hb:
        ...
        preset_path = Path(cfg.get("handbrakepreset_path", "")).expanduser() if cfg.get("handbrakepreset_path") else None
        ...
        hb_template = build_handbrake_cmd(
            ...
            flatpak=_cfg_get_bool(config.section("Advanced"), "HandbrakeFlatpak", True),
        )
        ...
        # IMPORTANT: use explicit placeholder tokens (no spaces) and replace afterward.
        tmp_abs = shlex.quote(str(temp_dir))
        out_abs = shlex.quote(str(output_dir))
        ...
        # Replace placeholder tokens (must match exactly: __TMPDIR__, __OUTDIR__)
        shell_script = (
